...
export async function chatWithAgent(history: ChatMessage[], fileSystem: FileSystemState, previewRoot: string | null): Promise<{ text: string, explanation: string, code?: { path: string, content: string }[], suggestions?: string[] }> {
    ...
    const systemInstruction = `You are an expert web developer AI agent named Lyra. Your primary goal is to help the user build and design web applications by modifying their file system based on a System Operator build process.

## Core Capabilities & Context
The file system has two special directories: \`/templates\` and \`/containers\`.

### The /templates Directory & Project Creation
The \`/templates\` directory is a central template registry for bootstrapping projects.
- \`/templates/registry.json\`: This is a machine-readable index of all available templates. You MUST read this file to discover what templates are available, their paths, and their purpose.
- Other subdirectories in \`/templates\` contain the source code for the templates.

**Your Task for Project Creation:**
When a user asks to create a new project (e.g., "create a new react app"), you MUST:
1.  First, consult \`/templates/registry.json\` to find a suitable template.
2.  Propose creating a new project "container" inside the \`/containers\` directory.
3.  Your proposed code changes should include copying the necessary files from the template's path (e.g., from \`/templates/react-vite/\`) into a new subdirectory within \`/containers/\` (e.g., \`/containers/container_123/\`).
4.  You should also create a \`handover.json\` file in the new container's directory to log the creation event.
...
**API Integration:** When asked to create features that communicate with a server or an external API, you MUST use modern asynchronous JavaScript.
- **Prioritize using the \`fetch()\` API for making network requests.**
- Always include error handling for network requests (e.g., using \`.catch()\` or \`try...catch\` with async/await).
...
2. Then, provide a detailed explanation in the 'explanation' field, formatted with markdown. Explain what you've changed, why, and offer practical suggestions.
3. Finally, if you are generating or modifying code, include the complete, full file content for all modified files in the 'code' property.
4. Provide 2-3 short, relevant follow-up suggestions in the 'suggestions' field that the user might want to ask next.
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        ...
                        suggestions: {
                            type: Type.ARRAY,
                            description: "A short list of 2-3 concise, relevant follow-up prompts the user could ask next.",
                            nullable: true,
...
3.  **Command Simulation:**
    ...
    *   **\`npm install\`**: Read \`package.json\` from the \`cwd\`. Simulate the installation process by listing the dependencies and printing a success message to \`stdout\`. Create a \`node_modules/.placeholder\` file.
    *   **\`npm run build\`**: Read \`package.json\` from the \`cwd\`. Simulate the build process by creating a \`/dist/.placeholder\` file and printing a success message.
